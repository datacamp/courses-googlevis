...
```{r,eval=FALSE}
library("rdatamarket")
load(url("http://s3.amazonaws.com/assets.datacamp.com/course/1007/datasets/ex2.RData"))
```
...
test_object("gdp_life_exp", msg = "Have you correctly joined `gdp` and `life_expectancy` to create `gdp_life_exp`?")
test_object("development", msg = "Have you correclty joined `gdp_life_exp` and `population` to create `development`?")
